$(document).ready(function() {
    ...
    function highlightText(id_highlight) {
        let content = $(`#${id_highlight}`).html();
    
        let cleanedContent = content.replace(/<span class="highlight">(.*?)<\/span>/g, "$1");
    
        let highlightedContent = cleanedContent.replace(new RegExp(search_input, "gi"), function(match) {
            return '<span class="highlight">' + match + '</span>';
        });
    
        $(`#${id_highlight}`).html(highlightedContent);
    }
    ...
    function display_search_results () {
        ...
        if (results.length === 0) {
            ...
            for (let id of results) {
                ...
                let name_highlight = `name-${court.id}-highlight`;
                let surfaces_highlight = `surfaces-${court.id}-highlight`;
                let types_highlight = `types-${court.id}-highlight`;
                ...
                let courtCard = `
                    <div class='col-md-4'>
                        <div class='court-card'>
                            ...
                            <h3 id='${name_highlight}' class='bold'>${court.name}</h3>
                            ...
                            <div class='preview-other'>
                                <span>
                                    <span class='bold'>Surfaces:</span>
                                    <span id='${surfaces_highlight}'>${court.surfaces}</span>
                                </span>
                                <span>
                                    <span class='bold'>Types:</span>
                                    <span id='${types_highlight}'>${court.types}</span>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                highlightText(name_highlight);
                highlightText(surfaces_highlight);
                highlightText(types_highlight);
            }
        }
    }
